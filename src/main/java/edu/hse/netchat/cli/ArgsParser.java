...
public final class ArgsParser {
  ...
  public CliArgs parse(String[] args) {
    ...
    Optional<InetSocketAddress> listen = Optional.ofNullable(options.get("--listen")).map(this::parseHostPort);
    Optional<InetSocketAddress> peer = Optional.ofNullable(options.get("--peer")).map(this::parseHostPort);
  ...
  public String helpText() {
    return "Usage: java -jar netchat.jar --name <username> [--listen host:port] [--peer host:port]\n"
        + "\n"
        + "Modes:\n"
        + "  - If --peer is set: connect to peer and start chat\n"
        + "  - If --peer is not set: listen (default listen is 0.0.0.0:0 if --listen omitted)\n";
  }
}
